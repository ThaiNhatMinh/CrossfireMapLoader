...
bool RenderBlock::Load(FILE * pFile)
{
	...
	if (nSectionCount)
	{
		for (uint32 nSectionLoop = 0; nSectionLoop < nSectionCount; ++nSectionLoop)
		{
			...
			//std::cout <<"sTextureName "<< sTextureName[0] << " " << sTextureName[1] << std::endl;
			...
			//std::cout << "sTextureEffect " << sTextureEffect << std::endl;
			// Set up the section
	...
	if (m_nVertexCount)
	{
		...
		Mesh->m_Vertexs.resize(m_nVertexCount);
		memcpy(&Mesh->m_Vertexs[0], m_aVertices, sizeof(SRBVertex)*m_nVertexCount);
	}
	...
	if (m_nTriCount)
	{
		...
		for (uint32 nTriLoop = 0; nTriLoop < m_nTriCount; ++nTriLoop)
		{
			...
			Mesh->m_Indices.push_back(nIndex0);
			Mesh->m_Indices.push_back(nIndex1);
			Mesh->m_Indices.push_back(nIndex2);
	...
	if(Mesh->m_Vertexs.size()) Mesh->Init();

	color.resize(m_aSections.size());
	/* initialize random seed: */
	srand(time(NULL));
	for (auto& el : color)
	{
		el = vec3((rand()%100)/100.0f, (rand() % 100) / 100.0f, (rand() % 100) / 100.0f);
	}
	return true;
}

void RenderBlock::Render(OpenGLRenderer* r, Camera * cam)
{
	m_pShader->Use();
	// ----- Transform Matricies ------

	m_pShader->SetUniformMatrix("Model", glm::value_ptr(mat4()));
	//mat4 view = glm::lookAt(vec3(0, 0, 200), vec3(0), vec3(0, 1, 0));
	mat4 MVP = cam->GetVPMatrix();
	m_pShader->SetUniformMatrix("MVP", glm::value_ptr(MVP));


	// ----- Lighting ------

	m_pShader->SetUniform("gLight.La", vec3(0.2));
	m_pShader->SetUniform("gLight.Ld", vec3(0.5));
	m_pShader->SetUniform("gLight.Ls", vec3(1.0f));
	m_pShader->SetUniform("gLight.direction", glm::normalize(vec3(-0.4,-1,0.5)));

	m_pShader->SetUniform("gMaterial.Ka", vec3(1.0f));
	m_pShader->SetUniform("gMaterial.Kd", vec3(1.0f));
	m_pShader->SetUniform("gMaterial.Ks", vec3(1.0f));
	m_pShader->SetUniform("gMaterial.exp", vec3(64.0f));

	m_pShader->SetUniform("EyePos", cam->GetPosition());
	
	m_pShader->SetUniform("ObjColor", vec3(1.0f));
	r->SetVertexArrayBuffer(Mesh->VAO);
	r->SetDrawMode(Mesh->Topology);
	for (size_t i = 0; i < m_aSections.size(); i++)
	{
		// ------- Render mesh ----------
		
		
		int ofset = m_aSections[i].m_nStartIndex * sizeof(uint32);
		r->DrawElement(m_aSections[i].m_nTriCount*3, GL_UNSIGNED_INT, (void*)ofset);
	}

	r->DrawLineBox(m_vBoundsMin, m_vBoundsMax);
}

void RenderBlock::SetWorld(RenderWorld * pWorld)
{
	m_pWorld = pWorld; m_pShader = m_pWorld->GetShader();
}
